...
## 1단계 — 로컬 개발 (전체 스택 로컬 구동 및 테스트 우선)
- **로컬 완결성 원칙**: 클라우드(Render/Supabase) 배포 이전에 로컬 환경에서 모든 기능과 연동 테스트가 독립적으로 완결되어야 한다.
- **통합 실행**: `docker-compose.yml` 기반으로 로컬 DB(PostgreSQL/pgvector) 및 각 서비스 컨테이너를 함께 띄워 전체 스택의 E2E 동작을 로컬에서 즉시 검증할 수 있어야 한다.
- **개발 생산성**: 볼륨 마운트로 핫리로드(`uvicorn --reload`, `npm run dev` 등)를 지원하여 빠른 피드백 루프를 유지한다.
- **격리된 테스트 환경**: 외부 클라우드 DB에 의존하지 않고, 로컬 테스트 DB 또는 Mock을 통해 로컬 및 CI 환경에서 모든 테스트(`pytest` 등)가 언제든 독립적으로 통과할 수 있어야 한다.
