...
- **PR 생성 규격 (Type B 템플릿 필수 준수)**: 사용자가 PR 생성을 요청하면 에이전트는 조직 공통 템플릿(`.github/pull_request_template.md`)의 Type B 규격을 100% 준수하여 본문을 작성한다:
  1. `## 🎯 작업 요약`: 목적과 주요 변경사항 명확화.
  2. `## 🌐 적용 범위`: 영향받는 API/도메인/컴포넌트 명시.
  3. `## 💬 고려사항 & 리뷰 포인트`: AI 바이브 코딩 시 고민한 설계 트레이드오프, 잠재적 사이드 이펙트, 사람이 집중 검토해야 할 포인트를 기술 (특별한 사항이 없는 경우 '해당 없음' 또는 '특이사항 없음' 기재).
  4. `## ✅ 체크리스트`: 작업 완료 항목을 `- [x]`로 체크.
  ※ 본문이 부실하거나 템플릿이 누락된 경우 중앙 CI(`reusable-pr-lint.yml`)에서 즉시 실패 처리되어 머지가 차단된다.
- **인간 개입 및 머지 권한**: 커밋 및 PR 초안 생성까지만 에이전트가 지원하며, `develop` 및 `main` 브랜치로의 **최종 PR 머지 버튼은 에이전트가 자율 실행하지 않고 사람이 직접 검토 후 누른다**.
...
- **세션 종료/인수인계**: 작업을 중단하거나 세션을 종료할 때 반드시 `.harness/HANDOFF.md`에 다음 세션을 위한 인수인계 서술을 남긴다.
- **중요 결정 기록**: 아키텍처나 정책의 중요한 결정은 `.harness/DECISIONS.md` 표 최상단에 이유와 함께 기록한다.
- **커밋**: 사용자가 명시적으로 요청했을 때만 수행하며, 변경된 파일만 선별해 스테이징한다 (`git add .` 지양). 커밋 본문이나 머지 시 Extended description은 필수가 아니며, 제목에 없는 새로운 정보(이유, 대안 비교 등)가 있을 때만 작성하고 제목을 그대로 복사해 채우지 않는다.
- **PR 생성**: PR을 열 때는 `.github/pull_request_template.md`의 구조(개요, 변경사항, 체크리스트)를 그대로 따라 본문을 채운다. 체크리스트 항목은 생략하지 않는다.
